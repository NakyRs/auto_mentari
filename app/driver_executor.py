...
def quiz(driver, nama_matkul, pertemuan, tipe, rand, key):
...
def execute(nama_matkul, nama_pert, tipe, rand, key):
    driver= set_driver()
    try:
        driver.get(link)
        WebDriverWait(driver, timeout=300).until(
        ...
        if tipe== 'Kuesioner':
            quisioner(driver, nama_matkul, nama_pert, rand)
        elif tipe == 'Posttest':
            quiz(driver, nama_matkul, nama_pert, tipe, rand, key)
